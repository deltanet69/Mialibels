...
const formatTime = (isoString?: string | null) => {
  ...
  // Try checking if it's already HH:MM format like "07:30"
  if (isoString.includes(':') && isoString.length <= 8) {
     return isoString.substring(0, 5) // return HH:MM
  }
...
export default function AbsenSiswaPage() {
  ...
  const fetchAttendanceList = async () => {
    try {
      ...
      const res = await fetch(`/api/attendance-siswa/list?className=${className}&date=${dateStr}&_t=${Date.now()}`)
      ...
      if (data.success && data.data) {
        ...
        const presentList = data.data.filter((s: any) => s.attendance && s.attendance.entry_time)
        // Sort by most recently checked in first
        presentList.sort((a: any, b: any) => {
           // entry_time is "HH:MM", we can compare string directly for sorting
           return b.attendance.entry_time.localeCompare(a.attendance.entry_time)
        })
        
        setPresentStudents(data.present_count)
    ...
    <main className="relative h-screen w-full overflow-hidden bg-white text-slate-800 font-sans flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-[35%] lg:w-[30%] min-w-[320px] max-w-md h-[40vh] md:h-full bg-white z-20 shadow-[10px_0_30px_rgba(0,0,0,0.1)] flex flex-col order-2 md:order-1 relative">
        <div className="p-6 lg:p-8 border-b border-slate-100 flex-shrink-0 bg-white">
          <h2 className="text-xl lg:text-2xl font-extrabold text-slate-800 tracking-tight">Daftar Kehadiran Siswa</h2>
          <p className="text-slate-500 font-medium mt-2">Total Siswa hadir <span className="font-bold text-slate-700">{presentStudents}/{totalStudents}</span></p>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
          {students.length === 0 ? (
            ...
            students.map((student, index) => (
              <div key={student.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center justify-between transition-all hover:shadow-md animate-in slide-in-from-left-4 fade-in duration-300">
                <div className="flex items-center gap-4 w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-blue-600 font-medium truncate">{student.class}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    <div className="flex items-center justify-between w-[80px]">
                      ...
                      <span className="text-xs font-semibold text-slate-700">{formatTime(student.attendance?.entry_time)}</span>
                    </div>
                    <div className="flex items-center justify-between w-[80px]">
                      <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wider">Keluar</span>
                      <span className="text-xs font-semibold text-slate-700">{formatTime(student.attendance?.exit_time)}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full px-4">
          ...
          <div className="text-center mb-6 animate-in slide-in-from-bottom-10 duration-700 delay-150 fade-in fill-mode-both">
            <h2 className="text-yellow-400 text-lg md:text-2xl font-bold tracking-widest uppercase mb-2">
              Absensi Siswa Kelas {className.toUpperCase()}
            </h2>
          ...
          >
            {popup.type === 'success' ? (
              <>
                ...
                {popup.student && (
                  <div className="flex flex-col items-center mb-6 w-full">
                    ...
                    />
                    <h3 className="text-xl lg:text-2xl font-bold text-slate-800 text-center">{popup.student.name}</h3>
                    <p className="text-blue-600 font-medium text-base lg:text-lg">{popup.student.class}</p>
                  </div>
                )}
              ...
              <>
                ...
                <div className="bg-red-50 text-red-700 px-6 py-3 rounded-xl font-semibold text-base lg:text-lg text-center w-full cursor-pointer" onClick={closePopup}>
                  SILAKAN COBA LAGI
                </div>
              </>
            )}
